...
export function Modal({
  ...
  return createPortal(
    <AnimatePresence>
      {open && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 xs:p-4">
          <motion.div
          ...
          <motion.div
            ...
            className={cn(
              "relative z-10 flex max-h-[calc(100dvh-1rem)] w-full flex-col xs:max-h-[calc(100dvh-2rem)]",
          ...
          >
            <button
              ...
              className="absolute right-2 top-2 grid size-9 place-items-center rounded-full text-ink-400 transition-colors hover:bg-ink-100 hover:text-ink-700 xs:right-4 xs:top-4"
            >
              <X className="size-5" />
            </button>
            ...
            {(title || description) && (
              <div className="shrink-0 px-3 pt-4 pr-12 xs:px-6 xs:pt-6 xs:pr-14">
                {title && (
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-3 py-4 xs:px-6 xs:py-5">
              {children}
            </div>

            {footer && (
              <div className="flex shrink-0 flex-wrap justify-end gap-2 border-t border-ink-100 px-3 py-3 xs:gap-3 xs:px-6 xs:py-4">
                {footer}
              </div>
            )}
          </motion.div>
